...
void UeStateLearner::execute_command(std::string msg) 
{
    ...
    if (!enableFuzzing) 
    {
        ...
        case MsgType::registrationRequestGUTI: {
            ...
            break;}
        case MsgType::plainRegistrationRequestGUTI: {
            std::cout << "sending plaintext registrationRequestGUTI" << std::endl;
            if (mm->m_storage->storedGuti->get().type == nas::EIdentityType::NO_IDENTITY)
            {
                notify_response("null_action");
                break;
            }
            // 用存储的 GUTI + FLAG_SECMOD=1 发明文注册（安全上下文建立后明文）
            FLAG_SECMOD = 1;
            mm->updateProvidedGuti();
            registrationRequestGUTI.mobileIdentity = mm->getOrGeneratePreferredId();
            if (mm->m_storage->lastVisitedRegisteredTai->get().hasValue())
        ...
        case MsgType::securityModeComplete:{
            ...
            else
            {
                ...
                // [ProbeFuzzer V2] 在 AMF 处理 SMC 并转 registered 之前（state S 窗口），
                // 立即发起 PDU 会话建立，用于验证 SM-before-MM 违规（OracleSmf）。
                if (getenv("V2_PDU_BEFORE_MM")) {
                    std::cout << "sending PDUSessionEstablishmentRequest after SMC (V2)" << std::endl;
    ...
    else
    {
        ...
        case MsgType::registrationRequestIMSI:
            ...
            break;
        case MsgType::plainRegistrationRequestGUTI: {
            std::cout << "sending plaintext registrationRequestGUTI (fuzzing)" << std::endl;
            if (mm->m_storage->storedGuti->get().type == nas::EIdentityType::NO_IDENTITY)
            {
                notify_response("null_action");
                break;
        ...
        case MsgType::incomingMessage:{
            ...
            if (nas::generate_bit(1))
            {
                ...
                // mutate_secmod sets FLAG_SECMOD; sendNasMessage reports WIRE sht/secmod/new_msg
                nas::mutate_secmod();
            ...
            // wait for CN downlink (max 2.5s); ret_type or ret_msg hex is enough
            for (int i = 0; i < 50; i++)
            {
                if (response->ret_type != "" || response->ret_msg != "")
                    break;
        ...
        case MsgType::testMessage:{
            ...
            downlink_notified = false;
            mm->sendNasMessageFromPDU(std::move(pdu));

            for (int i = 0; i < 50; i++)
            {
                if (downlink_notified)
                    break;
                std::this_thread::sleep_for(std::chrono::milliseconds(50));
            }
            if (!downlink_notified)
...
{
    if (msg != "OK" && msg != "Start fuzzing" && msg != "DONE")
        downlink_notified = true;
