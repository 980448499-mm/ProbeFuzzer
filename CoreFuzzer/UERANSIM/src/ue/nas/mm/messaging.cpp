...
EProcRc NasMm::sendNasMessage(const nas::PlainMmMessage &msg)
{
    ...
    // set security header type; report WIRE fields (not mutator intent)
    if (state_learner->enableFuzzing)
    {
        int sht = nas::gen_sht();
        ...
        {
            state_learner->response->sht = sht & 0x0F;
            state_learner->response->secmod = FLAG_SECMOD;
            state_learner->response->new_msg = pdu.toHexString();
        }
        printf("sht: %d\n", pdu.data()[1]);
        printf("secmod: %d\n", FLAG_SECMOD);
...
void NasMm::receiveMmMessage(const nas::PlainMmMessage &msg)
{
    ...
    else
    {
        if (state_learner->response != nullptr)
        {
            if (msg_name == "dlNasTransport")
            {
                // SM payload may already have filled ret_type via receiveSmMessage
                if (state_learner->response->ret_type.empty())
