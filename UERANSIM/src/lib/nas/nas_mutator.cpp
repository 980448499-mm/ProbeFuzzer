...
namespace nas
{
    ...
    int gen_sht()
    {
        // No security context: only plaintext SHT=0 is deliverable; other SHTs are dropped by AMF.
        if (!nr::ue::state_learner->has_sec_ctx())
            return 0;
        // 70%: SHT consistent with FLAG_SECMOD so the CN can parse the PDU.
        // 30%: valid 5GMM SHT 0-4 (includes plaintext-after-security, the high-value bypass probe).
        if (generate_int(10) < 7)
        {
            int sm = nr::ue::FLAG_SECMOD;
            if (sm <= 1)
                return 0;
            if (sm == 2)
                return generate_bit(1) ? 1 : 3; // integrity-protected
            return generate_bit(1) ? 2 : 4;     // integrity+ciphered
        }
