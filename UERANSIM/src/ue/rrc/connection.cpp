...
void UeRrcTask::receiveRrcRelease(const ASN_RRC_RRCRelease &msg)
{
    ...
    // [ProbeFuzzer] fuzzing harness 始终忽略 RRC Release、保持 RRC_CONNECTED。
    // 否则注册后连接被释放，后续 bypass seed（明文消息）会被当作 InitialUEMessage
    // 走初始注册路径（re-auth），无法到达已注册状态的故障注入点。
    (void)msg;
    m_logger->info("Ignoring RRC Release, keeping RRC_CONNECTED (fuzzing harness)");
}
